...
func canReachToGoalWithCards(_ cards1:[String], _ cards2:[String], _ goal:[String]) -> String {
    // 주석으로 된 부분은 cards1과 cards1의 카드를 모두 사용한 경우에는 해당되지만
    // 일부만 사용한 경우에는 적용되지 않으므로 기각

    //    if cards1.count + cards2.count == goal.count {
    //        let a = goal.filter {
    //            cards1.contains($0)
    //        }
    //
    ...
    //        }
    //
    //        return (a == cards1) && (b == cards2) ? "Yes" : "No"
    //    } else {
    var a = 0
    var b = 0
...
// MARK: - 본인이 생각하는 모범 답안
// reversed(), isEmpty, last, removeLast()의 시간복잡도는 O(1)
